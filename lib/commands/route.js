...
const { fs, routePath, allowedFileTypes, capitalize, templatePath } = require("./../helpers")
var ejs = require('ejs');
/**
 * 
 */
const createIndexFile = type => {
    const writeStream = fs.createWriteStream(path.join(routePath, `index.${type}`))
    const defaultContent = fs.readFileSync(`${templatePath}/${type}/routeIndex`, "utf8")
...
module.exports = args => {
    ...
    if (typeIndex > -1) {
        ...
        if (allowedFileTypes.indexOf(actualArgs[typeIndex + 1]) === -1) {
            throw new Error(`Please provide valid file type. Allowed types: ${allowedFileTypes}`);
        }
    ...
    const actualRouteName = routeName.indexOf("Route") > -1 ? `${capitalize(routeName)}` : `${capitalize(routeName)}Route`;
    // Create Route File
