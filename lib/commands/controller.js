...
const createIndexFile = type => {
  const writeStream = fs.createWriteStream(path.join(controllerPath, `index.${type}`));
  const defaultContent = fs.readFileSync(`${templatePath}/${type}/controllerIndex`, "utf-8");
...
module.exports = args => {
  ...
  const actualControllerName =
    controllerName.indexOf("Controller") > -1
